...
class PermissionToolNode(ToolNode):
    def _run_one(self, tool_call: dict, input_type: str, config) -> ToolMessage:
        ...
        trigger_hooks('PostToolUse',result)
        return result
...
def log_hook(tool_call):
    print(f"\033[90m[HOOK] PerToolUse: {tool_call.get('name')} called with {tool_call.get('args')}")
    return None  # 不阻断流程


def large_output_hook(tool_message:ToolMessage):
    output = getattr(tool_message,'content','') or ''
    name = getattr(tool_message,'content','') or ''
